...
public class ResourceXMLReader {
	 ...
	 */
	public static HashMap<Resource, Integer> readResourceXML(String path, Building building) {
		...
		try {
			...
			for (int i=0;i<nodes.getLength();i++) {
				if (Building.valueOf(nodes.item(i).getAttributes().getNamedItem("b").getNodeValue()).equals(building)) {
